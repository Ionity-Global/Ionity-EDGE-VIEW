...
void Paint_SetPixel(uint16_t Xpoint, uint16_t Ypoint, uint16_t Color)
{
    ...
    else if (Paint.Scale == 3)
    {
        uint8_t mask = 1u << (X % 8);
        for (uint8_t component = 0; component < 3; ++component)
        {
            unsigned int Addr = (X / 8) + Y * Paint.WidthByte + component * Paint.WidthByte * Paint.HeightByte;
            if (Color & (1u << component))
    ...
    else if (Paint.Scale == 3)
    {
        for (uint8_t component = 0; component < 3; ++component)
        {
            ...
            for (uint16_t Y = 0; Y < Paint.HeightByte; Y++)
            {
                for (uint16_t X = 0; X < Paint.WidthByte; X++)
                { // 8 pixel =  1 byte
                    uint32_t Addr = X + Y * Paint.WidthByte + component * Paint.WidthByte * Paint.HeightByte;
                    Paint.Image[Addr] = data;
                }
            }
        }
